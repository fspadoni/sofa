...
******************************************************************************/
#include "FreeMotionAnimationLoopAsync.h"
...
#include <MultiThreading/src/SolveVisitorAsync.h>
#include <MultiThreading/src/InitTasks.h>
...
FreeMotionAnimationLoopAsync::~FreeMotionAnimationLoopAsync()
{
    if (defaultSolver != NULL)
        defaultSolver.reset();
}

void FreeMotionAnimationLoopAsync::parse ( sofa::core::objectmodel::BaseObjectDescription* arg )
{
...
void FreeMotionAnimationLoopAsync::step(const sofa::core::ExecParams* params, SReal dt)
{
    if (dt == 0)
        dt = this->gnode->getDt();
...
void FreeMotionAnimationLoopAsync::initThreadLocalData()
{
	...
	for (int i = 0; i<nbThread; ++i)
	{
		...
		thread->addTask(new InitPerThreadDataTask(&atomicCounter, &InitPerThreadMutex, &status));
	}
...
int FreeMotionAnimationLoopClass = core::RegisterObject("Constraint solver")
        .add< FreeMotionAnimationLoopAsync >()
        .addAlias("FreeMotionMasterSolverMT")
        ;
