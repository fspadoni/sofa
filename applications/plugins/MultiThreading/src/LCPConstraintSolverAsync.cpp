...
#include "LCPConstraintSolverAsync.h"
...
namespace constraintset
{
    ...
    DEFINE_TASK_SCHEDULER_PROFILER(AddComplianceInConstraintSpaceTask);

    class AddComplianceInConstraintSpaceTask : public simulation::Task
    {
    public:
        AddComplianceInConstraintSpaceTask(core::behavior::BaseConstraintCorrection* cc, component::linearsolver::LPtrFullMatrix<double>  * W, core::ConstraintParams* cparams, Task::Status* status)
        ...
        virtual ~AddComplianceInConstraintSpaceTask()
        {
        }
    ...
    private:
        ...
        core::behavior::BaseConstraintCorrection* _cc;        
        component::linearsolver::LPtrFullMatrix<double>  *_W;
        core::ConstraintParams* _cparams;

    };

    DEFINE_TASK_SCHEDULER_PROFILER(ResetForUnbuiltResolutionTask);

    class ResetForUnbuiltResolutionTask : public simulation::Task
    {
    public:
        ResetForUnbuiltResolutionTask(core::behavior::BaseConstraintCorrection* cc, std::list<unsigned int>& contact_sequence, double * f, Task::Status* status)
            : simulation::Task(status)
            , _cc(cc)
        ...
        virtual ~ResetForUnbuiltResolutionTask()
        {            
        }
    ...
    private:
        ...
        core::behavior::BaseConstraintCorrection* _cc;
        std::list<unsigned int>& _contact_sequence;
        double * _f;
...
bool LCPConstraintSolverAsync::prepareStates(const core::ConstraintParams * /*cParams*/, MultiVecId /*res1*/, MultiVecId /*res2*/)
{
...
bool LCPConstraintSolverAsync::buildSystem(const core::ConstraintParams * /*cParams*/, MultiVecId /*res1*/, MultiVecId /*res2*/)
{
...
bool LCPConstraintSolverAsync::solveSystem(const core::ConstraintParams * /*cParams*/, MultiVecId /*res1*/, MultiVecId /*res2*/)
{
...
bool LCPConstraintSolverAsync::applyCorrection(const core::ConstraintParams * /*cParams*/, MultiVecId /*res1*/, MultiVecId /*res2*/)
{
...
LCPConstraintSolverAsync::LCPConstraintSolverAsync()
    : displayDebug(initData(&displayDebug, false, "displayDebug","Display debug information."))
    , displayTime(initData(&displayTime, false, "displayTime","Display time for each important step of LCPConstraintSolverAsync."))
...
void LCPConstraintSolverAsync::init()
{
...
void LCPConstraintSolverAsync::cleanup()
{
    if (!constraintCorrections.empty())
    {
        for (unsigned int i = 0; i < constraintCorrections.size(); i++)
...
void LCPConstraintSolverAsync::removeConstraintCorrection(core::behavior::BaseConstraintCorrection *s)
{
    constraintCorrections.erase(std::remove(constraintCorrections.begin(), constraintCorrections.end(), s), constraintCorrections.end());
}

void LCPConstraintSolverAsync::build_LCP()
{
    _numConstraints = 0;
    ...
    dmsg_info() <<"LCPConstraintSolverAsync: "<<_numConstraints<<" constraints, mu = "<<_mu ;
    ...
    for (unsigned int i = 0; i<constraintCorrections.size(); i++)
    {
        AddComplianceInConstraintSpaceTask*  task = new AddComplianceInConstraintSpaceTask(constraintCorrections[i], _W, &cparams,  &status);
        thread->addTask(task);
    }
...
void LCPConstraintSolverAsync::build_Coarse_Compliance(std::vector<int> &constraint_merge, int sizeCoarseSystem)
{
...
void LCPConstraintSolverAsync::MultigridConstraintsMerge()
{
...
void LCPConstraintSolverAsync::MultigridConstraintsMerge_Compliance()
{
...
void LCPConstraintSolverAsync::MultigridConstraintsMerge_Spatial()
{
...
void LCPConstraintSolverAsync::build_problem_info()
{
    core::ConstraintParams cparams;
...
void LCPConstraintSolverAsync::computeInitialGuess()
{
...
void LCPConstraintSolverAsync::keepContactForcesValue()
{
...
int LCPConstraintSolverAsync::nlcp_gaussseidel_unbuilt(double *dfree, double *f, std::vector<double>* residuals)
{
    if(!_numConstraints)
        return 0;
    ...
    simulation::Task::Status resetForUnbuiltResolutionTaskStatus;
    simulation::WorkerThread* thread = simulation::WorkerThread::getCurrent();

    for (unsigned int i = 0; i<constraintCorrections.size(); i++)
    {
        ResetForUnbuiltResolutionTask*  task = new ResetForUnbuiltResolutionTask(constraintCorrections[i], contact_sequence, f, &resetForUnbuiltResolutionTaskStatus);
        thread->addTask(task);
    }

    thread->workUntilDone(&resetForUnbuiltResolutionTaskStatus);
...
int LCPConstraintSolverAsync::lcp_gaussseidel_unbuilt(double *dfree, double *f, std::vector<double>* /*residuals*/)
{
    //helper::system::thread::CTime timer;
    double time = 0.0;
...
ConstraintProblem* LCPConstraintSolverAsync::getConstraintProblem()
{
    return last_lcp;
}

void LCPConstraintSolverAsync::lockConstraintProblem(sofa::core::objectmodel::BaseObject* /*from*/, ConstraintProblem* l1, ConstraintProblem* l2)
{
...
void LCPConstraintSolverAsync::draw(const core::visual::VisualParams* vparams)
{
...
int LCPConstraintSolverAsyncClass = core::RegisterObject("A Constraint Solver using the Linear Complementarity Problem formulation to solve BaseConstraint based components")
        .add< LCPConstraintSolverAsync >();

SOFA_DECL_CLASS(LCPConstraintSolverAsync);
